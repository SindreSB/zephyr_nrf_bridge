...
bool cc2500_verify_osc_stabilization()
{
	...
	do {
		cc2500_read_status_byte(&status);
...
void cc2500_set_mode(u8_t mode, u8_t checksum){
    u8_t state;
    cc2500_send_strobe(mode, &state);

    while((state & 0x70) != checksum) {
        k_busy_wait(1);
        cc2500_read_status_byte(&state);
    }
}

int cc2500_mode_idle() {
    // Ensure we are in idle mode first
    cc2500_set_mode(CC2500_CMD_SIDLE, 0x00);
...
int cc2500_mode_receive(u8_t channel){
    ...
    cc2500_send_strobe(CC2500_CMD_SFRX, NULL);

    printk("MR: set channel and freq\n");
    // Write the channel dependent offset and channel number
    cc2500_write_register(CC2500_REG_CHANNR, nChannels[channel], NULL);
    cc2500_write_register(CC2500_REG_FSCTRL0, fOffset[channel], NULL);

    // Set the chip into RX mode
    printk("MR: set rx mode\n");
    cc2500_set_mode(CC2500_CMD_SRX, 0x10);
